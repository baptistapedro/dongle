...
ADD . /dongle
WORKDIR /dongle
ADD fuzzers/fuzz_decode.go ./fuzzers/
WORKDIR ./fuzzers/
RUN go mod init myfuzz
RUN go install github.com/dvyukov/go-fuzz/go-fuzz@latest github.com/dvyukov/go-fuzz/go-fuzz-build@latest
RUN go get github.com/dvyukov/go-fuzz/go-fuzz-dep
RUN go get github.com/golang-module/dongle
RUN /root/go/bin/go-fuzz-build -libfuzzer -o harness.a
RUN clang -fsanitize=fuzzer harness.a -o fuzz_decode

FROM ubuntu:20.04
COPY --from=builder /dongle/fuzzers/fuzz_decode /
